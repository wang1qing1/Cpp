...
public:
    ...
    vector<int> eatGrass(vector<vector<int>>& matrix) {
        ...
        int left=0;int top=0;int right=matrix[0].size()-1;int down=matrix.size()-1;
        while(count--)
        {
            for(int i=down;left<=right&&i>=top;i--)ret.push_back(matrix[i][left]);
            left++;
            for(int i=left;top<=down&&i<=right;i++)ret.push_back(matrix[top][i]);
            top++;
            for(int i=top;left<=right&&i<=down;i++)ret.push_back(matrix[i][right]);
            right--;
